...
export function useRequest<Response, Payload extends unknown[]>(
  ...
  async function runAsync(...args: Payload): Promise<void> {
    try {
      ...
      result.value = res
      onSuccess?.(unref(result), args)
    } catch (err: unknown) {
      error.value = err
      onFailure?.(err as Error, args)
    } finally {
      loading.value = false
      onAfter?.(args)
    }
  ...
  function runPrimitive(): void {
    runAsync(...defaultParams as Payload)
  }
...
type QueryService<Response, Payload extends unknown[]> = (...args: Payload) => Promise<Response>
